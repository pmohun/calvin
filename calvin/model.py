...
def load_model(api_key, model):
    if model == 'openai':
      calvin.api_key = api_key
      calvin.headers = {
...
def generate(prompt):
  url = f"https://api.openai.com/v1/engines/{calvin.engine}/completions"
  if prompt in list_prompts():
    prompts = load_json()
    payload = prompts[prompt][0]
    response = requests.post(url, headers=calvin.headers, json=payload).json()
  else:
    payload = {
      "prompt": prompt,
      "max_tokens": 20,
...
#TODO Determine our approach here. Do we want to stick with classes? When is the right time to do so?
...
class Model:
  ...
  def generate(self, prompt):
    ...
    if prompt in list_prompts():
      payload = self.prompts[payload][0]
